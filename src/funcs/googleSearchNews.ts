 ...
 */
export async function googleSearchNews(
  client: ExfuncCore,
  request: operations.SearchNewsRequestBody,
  options?: RequestOptions,
): Promise<
  Result<
    operations.SearchNewsResponseBody,
    | errors.UserError
    | errors.ServerError
    | SDKError
    | SDKValidationError
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.SearchNewsRequestBody$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/google/search-news")();
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, operations.SearchNewsResponseBody$inboundSchema),
