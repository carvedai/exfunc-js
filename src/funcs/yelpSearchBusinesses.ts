 ...
 */
export async function yelpSearchBusinesses(
  client: ExfuncCore,
  request: operations.SearchBusinessesRequestBody,
  options?: RequestOptions,
): Promise<
  Result<
    operations.SearchBusinessesResponseBody,
    | errors.UserError
    | errors.ServerError
    | SDKError
    | SDKValidationError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.SearchBusinessesRequestBody$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/yelp/search-businesses")();
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, operations.SearchBusinessesResponseBody$inboundSchema),
