 ...
 */
export async function linkedinGetPerson(
  client: ExfuncCore,
  request: operations.GetPersonRequestBody,
  options?: RequestOptions,
): Promise<
  Result<
    operations.GetPersonResponseBody,
    | errors.UserError
    | errors.ServerError
    | SDKError
    | SDKValidationError
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetPersonRequestBody$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/linkedin/get-person")();
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, operations.GetPersonResponseBody$inboundSchema),
