 ...
 */
export async function linkedinSearchJobPostings(
  client: ExfuncCore,
  request: operations.SearchJobPostingsRequestBody,
  options?: RequestOptions,
): Promise<
  Result<
    operations.SearchJobPostingsResponseBody,
    | errors.UserError
    | errors.ServerError
    | SDKError
    | SDKValidationError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.SearchJobPostingsRequestBody$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/linkedin/search-job-postings")();
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, operations.SearchJobPostingsResponseBody$inboundSchema),
