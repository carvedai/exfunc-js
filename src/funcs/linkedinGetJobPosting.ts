 ...
 */
export async function linkedinGetJobPosting(
  client: ExfuncCore,
  request: operations.GetJobPostingRequestBody,
  options?: RequestOptions,
): Promise<
  Result<
    operations.GetJobPostingResponseBody,
    | errors.UserError
    | errors.ServerError
    | SDKError
    | SDKValidationError
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetJobPostingRequestBody$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/linkedin/get-job-posting")();
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, operations.GetJobPostingResponseBody$inboundSchema),
