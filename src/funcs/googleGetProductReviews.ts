 ...
 */
export async function googleGetProductReviews(
  client: ExfuncCore,
  request: operations.GetProductReviewsRequestBody,
  options?: RequestOptions,
): Promise<
  Result<
    operations.GetProductReviewsResponseBody,
    | errors.UserError
    | errors.ServerError
    | SDKError
    | SDKValidationError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetProductReviewsRequestBody$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/google/get-product-reviews")();
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, operations.GetProductReviewsResponseBody$inboundSchema),
