 ...
 */
export async function twitterGetUserFollowings(
  client: ExfuncCore,
  request: operations.GetUserFollowingsRequestBody,
  options?: RequestOptions,
): Promise<
  Result<
    operations.GetUserFollowingsResponseBody,
    | errors.UserError
    | errors.ServerError
    | SDKError
    | SDKValidationError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetUserFollowingsRequestBody$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/twitter/get-user-followings")();
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, operations.GetUserFollowingsResponseBody$inboundSchema),
