 ...
 */
export async function twitterGetUserTweets(
  client: ExfuncCore,
  request: operations.GetUserTweetsRequestBody,
  options?: RequestOptions,
): Promise<
  Result<
    operations.GetUserTweetsResponseBody,
    | errors.UserError
    | errors.ServerError
    | SDKError
    | SDKValidationError
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetUserTweetsRequestBody$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/twitter/get-user-tweets")();
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, operations.GetUserTweetsResponseBody$inboundSchema),
