 ...
 */
export function twitterSearchTweets(
  client: ExfuncCore,
  request: operations.SearchTweetsRequestBody,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.SearchTweetsResponseBody,
    | errors.UserError
    | errors.ServerError
    | ExfuncError
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.SearchTweetsRequestBody$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/twitter/search-tweets")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.SearchTweetsResponseBody$inboundSchema),
